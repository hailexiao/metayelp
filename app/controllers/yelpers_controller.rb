...
class YelpersController < ApplicationController
  ...
  def create
    if params[:yelper][:uid].include?("userid=")
      ...
      params[:yelper][:image_url] = doc.css(".photo-slideshow_image img").attr('src').text

      params[:yelper][:number_of_reviews] = doc.css(".review-count span strong").text
  ...
  private
  def yelper_params
    params.require(:yelper).permit(:name, :location, :image_url, :number_of_reviews, :uid)
  end
